...
{
    // printf("rtc-board->BlockLowPowerDuringTask");
    if( status == true )
    {
        RtcRecoverMcuStatus( );
    }
...
void RtcEnterLowPowerStopMode( void )
{
    if( ( LowPowerDisableDuringTask == false ) && ( RtcTimerEventAllowsLowPower == true ) )
    {
        ...
        // Enter Stop Mode
        //***LArge delay here. Nothing to worry about. Not an issue
...
{
  // printf("rtc-board->RtcRecoverMcuStatus");
...
static void RtcComputeWakeUpTime( void )
{
  // printf("rtc-board->RtcComputeWakeUpTime");
...
static void RtcStartWakeUpAlarm( uint32_t timeoutValue )
{
  // printf("rtc-board->RtcStartWakeUpAlarm");
